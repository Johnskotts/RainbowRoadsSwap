    ...
    <div className={styles.main}>
      <div className={page == 0 ? styles.swap : styles.swap2}>
        <div className={styles.header}>
          <button onClick={() => setModal(!modalStatus)}>
            <img src="/cross.png" alt="cross" />
          </button>
          <div>
            RAINBOW <span>ROADS</span>
          </div>
        </div>
        {page == 0 ? (
          <div className={styles.swap_form}>
            <div className={styles.swap_1}>
              ...
              <div className={styles.side_box}>
                <div
                  className={styles.emoji_selector_1}
                  onClick={() => setFromShowTokenSelector(true)}
                >
                  {fromToken.symbol}
                </div>
                <div className={styles.liqudity}>
                  <span>Liq:</span> <span>{fromToken.liquidity}</span>{" "}
                  <span>{tokenList[0].symbol}</span>
                </div>
              </div>
            </div>
            <div className="swapbutton">
              ...
              >
                <svg
                  ...
                  className="lucide lucide-arrow-up-down w-4 h-4 text-black"
                >
            ...
            <div className={styles.swap_1}>
              ...
              <div className={styles.side_box}>
                <div
                  className={styles.emoji_selector_1}
                  onClick={() => setToShowTokenSelector(true)}
                >
                  {toToken.symbol}
                </div>
                <div className={styles.liqudity}>
                  <span>Liq:</span> <span>{toToken.liquidity}</span>{" "}
                  <span>{tokenList[0].symbol}</span>
                </div>
              </div>
            </div>
        ...
        {showFromTokenSelector && (
          <TokenSelector
            ...
            handleClose={() => setFromShowTokenSelector(false)}
          />
        )}
        {showToTokenSelector && (
          <TokenSelector
            setToken={setToToken}
            handleClose={() => setToShowTokenSelector(false)}
          />
        )}
      </div>
    </div>
